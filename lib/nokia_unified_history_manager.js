...
class NokiaUnifiedHistoryManager {
    ...
    /**
     * ✅ NEW: Clear ONLY ChatGPT text messages from a context
     * Keeps SMS (messages app) and voice call (phone app) history intact
     * @param {string} contextId - Context ID
     */
    clearChatGPTMessages(contextId) {
        if (!this.contexts[contextId]) {
            this.contexts[contextId] = [];
            return;
        }

        // Filter out only ChatGPT text messages
        const beforeCount = this.contexts[contextId].length;
        this.contexts[contextId] = this.contexts[contextId].filter(msg => {
            // Keep if NOT a ChatGPT text message
            const isChatGPTText = (
                msg.type === 'text' && 
                msg.metadata && 
                msg.metadata.app === 'chatgpt'
            );
            return !isChatGPTText; // Keep everything else (SMS, voice calls, etc.)
        });

        const afterCount = this.contexts[contextId].length;
        const deletedCount = beforeCount - afterCount;

        this.saveHistories();
        console.log(`🗑️ ChatGPT messages cleared from context "${contextId}": ${deletedCount} deleted, ${afterCount} kept (SMS/voice)`);
    }
