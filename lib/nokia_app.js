...
// Storage functions
// ✅ Make loadFromStorage globally accessible
...
async function loadFromStorage() {
    try {
        // ✅ CSAK a beállításokat töltjük be, a history-t NEM
        // A history már a unified history manager-ben van!
        const savedInput = localStorage.getItem(STORAGE_KEYS.INPUT);
        const savedCursor = localStorage.getItem(STORAGE_KEYS.CURSOR);
        const savedModel = localStorage.getItem(STORAGE_KEYS.MODEL);
        const savedVoiceModel = localStorage.getItem(STORAGE_KEYS.VOICE_MODEL);
...
function saveToStorage() {
    try {
        // ✅ CSAK a beállításokat mentjük, a history-t NEM
        // A history automatikusan mentődik a unified history manager-ben!
...
async function restoreMessages() {
    ...
    // ✅ JAVÍTÁS: CSAK a ChatGPT text üzeneteket jelenítjük meg!
    // SMS és voice call üzenetek NEM jelennek meg a ChatGPT app-ban
    const textMessages = window.conversationHistory.filter(msg => {
        return msg.type === 'text' && msg.metadata && msg.metadata.app === 'chatgpt';
    });
...
// ✅ Make restoreMessages globally accessible
...
async function playDTMF(key) {
    ...
    // ✅ Ellenőrizzük, hogy running-e (kellene lennie a capture listener miatt)
    if (audioContext.state !== 'running') {
        //console.error('❌ AudioContext is NOT running:', audioContext.state);
        //console.error('   This should not happen! Check capture phase listeners.');
        
        // ✅ FALLBACK: Próbáljuk meg még egyszer feloldani (valami hibás lehet)
        try {
            await audioContext.resume();
            //console.warn('⚠️ Fallback resume succeeded');
        } catch (err) {
            console.error('❌ Fallback resume failed:', err);
            return;
        }
        
        // Még mindig nem running?
        if (audioContext.state !== 'running') {
            console.error('❌ AudioContext STILL not running after fallback');
            return;
        }
    }
    
    // ✅ Ha itt vagyunk, akkor a context biztos running, jöhet a hang!
